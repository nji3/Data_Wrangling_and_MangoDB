...
def main():
    ...
    num_results = len([i for i,c in enumerate(results["artists"]) if c['name'].capitalize()==('FIRST AID KIT').capitalize()])
    print(num_results) # There are 2
    ...
    print(results['artists'][0]['aliases']) ## Los Beatles

    ## Question 4
    # Query for information in the database about bands named NIRVANA
    results = query_by_name(ARTIST_URL, query_type["simple"], "NIRVANA")
    # NIRVANA disambiguation?
    print(results['artists'][0]['disambiguation']) # 90s US grunge band

    ## Question 5
    # Query for information in the database about bands named ONE DIRECTION
    results = query_by_name(ARTIST_URL, query_type["simple"], "One Direction")
    # When was one direction formed?
    print(results['artists'][0]['life-span']['begin'].split('-')[0]) # 2010

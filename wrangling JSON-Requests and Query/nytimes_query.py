...
def article_overview(kind, period):
    ...
    # YOUR CODE HERE
    data = get_from_file(kind, period)
    # titles = [{asset['section']:asset['title']} for asset in data]
    for asset in data:
        titles.append({asset['section']:asset['title']})
        media = asset['media']
        for m in media:
            media_dat = m['media-metadata']
            for url_dic in media_dat:
                if url_dic['format'] == "Standard Thumbnail":
                    urls.append(url_dic['url'])
